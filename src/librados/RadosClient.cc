...
int librados::RadosClient::mgr_command(vector<string>&& cmd,
				       ...
				       bufferlist *outbl, string *outs)
{
  ldout(cct, 0) << "dror: in mgr_command" << dendl;
  std::lock_guard l(lock);
...
int librados::RadosClient::mgr_command(
  ...
  bufferlist *outbl, string *outs)
{
  ldout(cct, 0) << "dror: in mgr_command2" << dendl;
  std::lock_guard l(lock);
...
int librados::RadosClient::mon_command(int rank, vector<string>&& cmd,
				       ...
				       bufferlist *outbl, string *outs)
{
  ldout(cct, 0) << "dror: in mon_command" << dendl;
  bs::error_code ec;
...
int librados::RadosClient::mon_command(std::string&& name, vector<string>&& cmd,
				       ...
				       bufferlist *outbl, string *outs)
{
  ldout(cct, 0) << "dror: in mon_command2" << dendl;
  bs::error_code ec;
...
{
  ldout(cct, 0) << "dror: in osd_command" << dendl;
  ceph_tid_t tid;
...
int librados::RadosClient::pg_command(pg_t pgid, vector<string>&& cmd,
				      ...
				      bufferlist *poutbl, string *prs)
{
  ldout(cct, 0) << "dror: in pg_command" << dendl;
  ceph_tid_t tid;
  bs::error_code ec;
