...
bool PrimaryLogPG::check_laggy(OpRequestRef& op)
{
  ...
  size_t unreadable_cout = (it != waiting_for_unreadable_object.end()) ? it->second.size() : 0;
  bool trigger_laggy = !m_disable_hook && m->get_oid().name == "my_test_obj" && op->may_write() && unreadable_cout;
  ...
  << ", unreadable_cout = " << unreadable_cout << ", m_laggy_hook_op_count = " << m_laggy_state_op_count << ", m_disable_hook = " << m_disable_hook << dendl;
  ...
  if (trigger_laggy) {
      dout(10) << __func__ << " trigger_laggy kicking back " << unreadable_cout << " ops from unreadable list" << dendl;
      requeue_ops(waiting_for_unreadable_object[m->get_hobj()]);

      m_laggy_state_op_count = unreadable_cout;
  }

  if (m->get_oid().name == "my_test_obj" && op->may_read()) {
      m_laggy_state_op_count--;
      dout(10) << __func__ << " trigger_laggy read hook: m_laggy_hook_op_count = " << m_laggy_state_op_count << dendl;
      if (m_laggy_state_op_count == 0) {
...
void PrimaryLogPG::do_op(OpRequestRef& op)
{
  ...
  if (!m_disable_hook && g_conf().get_val<bool>("osd_reproduce_bug_75403_hook") && op->may_read() &&
    m->get_oid().name == "my_test_obj") {
...
void PrimaryLogPG::execute_ctx(OpContext *ctx)
{
  ...
  dout(20) << __func__ << " before out of order check: cct->_conf->osd_debug_op_order  = " << cct->_conf->osd_debug_op_order
